...
func (emitter *Emitter) writeBlockScalarHints(value []byte) error {
	...
	if i < len(value) && isSpace(value, i) {
		...
		// emitting BestIndent produces an unparseable document. Use the actual
		// difference, falling back to BestIndent when it is out of the 1..9
		// range the indicator can express.
